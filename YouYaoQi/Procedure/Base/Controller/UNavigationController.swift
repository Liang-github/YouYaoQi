...
class UNavigationController: UINavigationController {
    ...
    override func viewDidLoad() {
        ...
        guard let internalTagets = interactionGes.value(forKeyPath: "targets") as? [NSObject] else {
            return
        }
        guard let internalTarget = internalTagets.first?.value(forKey: "target") else {
            return
        }
...
extension UNavigationController: UIGestureRecognizerDelegate {
    func gestureRecognizerShouldBegin(_ gestureRecongnizer: UIGestureRecognizer) -> Bool {
        let isLeftToRight = UIApplication.shared.userInterfaceLayoutDirection == .leftToRight
        guard let ges = gestureRecongnizer as? UIPanGestureRecognizer else {
            return true
        }
        
        if ges.translation(in: gestureRecongnizer.view).x*(isLeftToRight ? 1 : -1) <= 0 || value(forKey: "_isTransitioning") as! Bool || disablePopGesture {
            return false
        }
